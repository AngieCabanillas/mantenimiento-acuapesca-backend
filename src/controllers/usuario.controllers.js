  ...
  try {
    if (req.query.keyword && req.query.page) {
      const data = await pool.query(
        `SELECT * FROM usuario WHERE (nombres_usuario LIKE '%${req.query.keyword}%' OR apellidos_usuario LIKE '%${req.query.keyword}%' OR email_usuario LIKE '%${req.query.keyword}%') ORDER BY id_usuario DESC`
      );
      const cantidadDatos = 12;
      const pagina = (parseInt(req.query.page) - 1) * cantidadDatos;
      return res.json({
        success: "Datos obtenidos",
        usuarios: data.splice(pagina, cantidadDatos),
      });
    }
...
ctrlUsuario.createUsuario = async (req, res) => {
  ...
  const password = await helpers.encryptPassword(
    nombres_usuario.toLowerCase().replace(/ \s*/g, "") + dni
  );
  const newUsuario = {
    nombres_usuario,
    apellidos_usuario,
  ...
  try {
    ...
    if (rows.affectedRows === 1)
      return res.json({ success: "Usuario Creado." });
    return res.json({ error: "Ocurrió un error." });
  } catch (error) {
    console.log(error);
    ...
    if (error.code === "ER_DUP_ENTRY") {
      let fila;
      error.sqlMessage.indexOf("dni") === -1
        ? (fila = "Correo")
        : (fila = "DNI");
...
ctrlUsuario.updateUsuario = async (req, res) => {
  ...
  try {
    const rows = await pool.query("UPDATE usuario set ? WHERE id_usuario = ?", [
      newUsuario,
      req.params.id,
    ]);
    if (rows.affectedRows === 1)
...
ctrlUsuario.deleteUsuario = async (req, res) => {
  try {
    ...
    rows[0].estado_usuario == 0
      ? (rows[0].estado_usuario = 1)
      : (rows[0].estado_usuario = 0);
    const data = await pool.query("UPDATE usuario set ? WHERE id_usuario = ?", [
      rows[0],
      req.params.id,
    ]);
    let estado = "";
    ...
    if (data.affectedRows === 1)
      return res.json({
        success: `Usuario ${rows[0].nombres_usuario} ${estado}`,
      }); //Se logró actualizar
    return res.json({ error: "Ocurrió un error" });
  } catch (error) {
    console.log(error);
